...
class CrackTask:
    ...
    def get_error_length(self):
        ...
        self.conn = conn
        # pre_res = self.crack_request(conn, self.test_username, self.test_password)
        # 预请求一次,判断失败条件，如果失败长度在误差范围内则视为一致
        res0 = self.crack_request(conn, self.test_username, self.test_password) #第一次请求获取cookie
        ...
        if (error_length1 - error_length2) > 10 or (error_length1 - error_length2) < -10:
